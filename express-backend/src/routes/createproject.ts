...
        const roleInsertQuery = `INSERT INTO project_roles(project_id, role_name) VALUES ${roleValues}`;
...
      // Fetch project roles
      const rolesQuery = `SELECT id, role_name FROM project_roles WHERE project_id = $1`;
...
        const roleInsertQuery = `INSERT INTO project_roles(project_id, role_name) VALUES ${roleValues}`;
          
        const roleParams: (string | number)[] = newRoles.flatMap((role) => [
          parseInt(projectId),
          role,
        ]);
        await client.query(roleInsertQuery, roleParams);
