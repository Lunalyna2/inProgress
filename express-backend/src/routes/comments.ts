...
router.post("/projects/:projectId/comments", authMiddleware, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    const avatarResult = await pool.query("SELECT avatar FROM userprofile WHERE user_id = $1", [userId]);
    newComment.avatar = avatarResult.rows[0]?.avatar || null;
...
router.put("/comments/:id", authMiddleware, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    const avatarResult = await pool.query("SELECT avatar FROM userprofile WHERE user_id = $1", [userId]);
    updatedComment.avatar = avatarResult.rows[0]?.avatar || null;
