...
const pool_1 = __importDefault(require("../pool")); // adjust import path if different
const router = (0, express_1.Router)();
const FRONTEND_URL = process.env.FRONTEND_URL || "http://localhost:3000";
router.post("/forgot-password", async (req, res) => {
    try {
        const { cpuEmail } = req.body;
        if (!cpuEmail)
            return res.status(400).json({ message: "Email is required." });
        const userResult = await pool_1.default.query("SELECT id FROM users WHERE email = $1", [cpuEmail]);
        ...
        await pool_1.default.query("UPDATE users SET reset_token = $1, reset_token_expires = $2 WHERE id = $3", [resetToken, expires, userId]);
        ...
        const resp = { message: "Password reset link sent (if configured)." };
...
router.post("/reset-password/:token", async (req, res) => {
    try {
        ...
        const userResult = await pool_1.default.query("SELECT id FROM users WHERE reset_token = $1 AND reset_token_expires > NOW()", [token]);
        if (userResult.rows.length === 0)
            return res.status(400).json({ message: "Invalid or expired token." });
        const userId = userResult.rows[0].id;
        const hashed = await require("bcrypt").hash(newPassword, 10);
        await pool_1.default.query("UPDATE users SET password = $1, reset_token = NULL, reset_token_expires = NULL WHERE id = $2", [hashed, userId]);
