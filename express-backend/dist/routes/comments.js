...
const pool_1 = __importDefault(require("../pool")); // your PostgreSQL pool
const shared_1 = require("../shared");
const router = express_1.default.Router();
router.get("/projects/:projectId/comments", shared_1.authMiddleware, async (req, res) => {
...
router.post("/projects/:projectId/comments", shared_1.authMiddleware, async (req, res) => {
    const projectId = Number(req.params.projectId);
    const { text } = req.body;
    ...
    try {
        ...
        // Fetch avatar
        const avatarQuery = await pool_1.default.query("SELECT avatar FROM userprofile WHERE user_id = $1", [userId]);
...
router.put("/comments/:id", shared_1.authMiddleware, async (req, res) => {
    ...
    try {
        const result = await pool_1.default.query(`UPDATE comments
             ...
             RETURNING id, text, updated_at, user_id`, [text, commentId, userId]);
...
router.delete("/comments/:id", shared_1.authMiddleware, async (req, res) => {
    const commentId = Number(req.params.id);
    const userId = req.userId;
    if (isNaN(commentId))
